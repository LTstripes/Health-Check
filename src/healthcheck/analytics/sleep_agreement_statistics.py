"""Deterministic R05 agreement statistics and evidence gates.

This is the pure, read-only statistics surface for #103.  The #102
``sleep_agreement`` module remains the persistence/replay boundary; this
module deliberately does not persist runs, select a canonical source, or
switch canonical data.

The pinned conventions are ``difference = google - garmin``, sample SD,
parametric limits of agreement at ``bias +/- 1.96 * sample_sd``, and
Hyndman-Fan Type-7 quantiles at 2.5% and 97.5%.  Tukey 1.5-IQR output is
sensitivity metadata only.  The 14-night exploratory and 42-night
provisional gates are the only N gates; coverage and max-gap values are
reported evidence, never automatic thresholds.
"""
...
class EpochBasis:
    """Typed, reviewable evidence for a real measurement epoch boundary."""
...
class AgreementEpoch:
    """Epoch identity; non-default identities require a typed break basis."""
...
class AgreementObservation:
    """One metric-night for exploratory and strong evidence calculations."""
    ...
    _projection_evidence_verified: bool = field(default=False, repr=False, compare=False)

    def __post_init__(self) -> None:
        object.__setattr__(self, "wake_date", _as_date(self.wake_date))
        metric_code = self.metric_code.strip()
        cohort = self.cohort.strip()
        if isinstance(self.epoch, str) and self.epoch_basis is not None:
            epoch = AgreementEpoch(self.epoch, self.epoch_basis)
        else:
        ...
        if google is not None and garmin is not None:
            ...
            difference_verified = bool(self._projection_evidence_verified)
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "difference_verification_basis": (
                "source_values"
                if self.google_value is not None and self.garmin_value is not None
                else "projection_evidence"
                if self.difference_verified
                else None
            ),
        }
    ...
    def from_projection(
        ...
        *,
        ...
        return cls(
            ...
            google_value=getattr(getattr(projection, "google", None), "value", None),
            garmin_value=getattr(getattr(projection, "garmin", None), "value", None),
            ...
            _projection_evidence_verified=True,
        )
...
def observations_from_projections(
    ...
    *,
    ...
    """Adapt accepted #101 projections without copying raw payloads."""
...
class AgreementStatistics:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            "contract_version": R05_SLEEP_AGREEMENT_STATISTICS_CONTRACT_VERSION,
            "rule_version": R05_SLEEP_AGREEMENT_STATISTICS_RULE_VERSION,
            "algorithm": R05_SLEEP_AGREEMENT_STATISTICS_ALGORITHM,
            "metric_code": self.metric_code,
            "cohort": self.cohort,
            "epoch": self.epoch,
            "epoch_basis": self.epoch_basis.as_dict() if self.epoch_basis else None,
...
def _correlation(observations: Sequence[AgreementObservation]) -> tuple[float | None, float | None]:
    ...
    if len(set(google)) < 2 or len(set(garmin)) < 2:
        return None, None
    return (
        float(statistics.correlation(google, garmin)),
        float(statistics.correlation(_midranks(google), _midranks(garmin))),
    )
...
def _outlier_sensitivity(
    ...
    retained = [
        float(item.difference) for wake_date, item in selected if wake_date not in outlier_set
    ]
...
def _coverage(
    ...
    *,
    ...
    if requested_start_date is not None and requested_end_date is not None:
        if window_dates:
            if window_dates[0] > requested_start_date:
                end = window_dates[0] - timedelta(days=1)
                gaps.append(
                    AgreementGap(
                        requested_start_date,
                        end,
                        (end - requested_start_date).days + 1,
                    )
                )
            if window_dates[-1] < requested_end_date:
                start = window_dates[-1] + timedelta(days=1)
                gaps.append(
                    AgreementGap(
                        start,
                        requested_end_date,
                        (requested_end_date - start).days + 1,
                    )
                )
        else:
            gaps.append(
    ...
    return AgreementCoverage(
        ...
        selected_dates,
        tuple(gaps),
        max((item.missing_days for item in gaps), default=None),
    )
...
def _half(selected: Sequence[tuple[date, AgreementObservation]]) -> AgreementHalfStatistics:
    ...
    return AgreementHalfStatistics(
        ...
        _calendar_span_inclusive(dates[0], dates[-1]) if dates else None,
        _calendar_span_delta(dates[0], dates[-1]) if dates else None,
        summary[0],
        summary[1],
        summary[2],
        summary[3],
...
def _stability(
    ...
    *,
    ...
    return AgreementStability(
        ...
        None if passed else "half_robust_distributions_not_stable",
    )
...
def _gate(
    *,
    ...
    if cohort == ACCOUNT_WEARABLES_SLEEP_OBSERVATIONS:
        ...
        reasons.append(
            "provisional_n_below_42" if n < PROVISIONAL_MIN_N else "strong_gate_n_below_42"
        )
...
def _coerce_observations(
    source: Iterable[AgreementObservation] | Any,
) -> tuple[AgreementObservation, ...]:
...
def compute_agreement_statistics(
    ...
    *,
    ...
    if any(bool(item.canonical_candidate) != canonical_candidate for item in selected_observations):
    ...
    return AgreementStatistics(
        ...
        n=len(values),
        strong_gate_eligible_n=len(strong_deduped),
        google_manually_edited_n=sum(item.google_manually_edited is True for _, item in deduped),
        wake_dates=dates,
