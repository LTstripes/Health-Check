...
def test_incomplete_activity_fetch_preserves_previous_successful_checkpoint(
    tmp_path: Path, monkeypatch
):
    client = _client(activities=[_activity(101)])
    ...
    try:
        with factory() as session:
            state = session.scalar(
                select(SyncStreamState).where(SyncStreamState.stream_code == "activities")
            )
            previous = (state.cursor, state.watermark, state.last_success_at)
            assert all(value is not None for value in previous)
    finally:
        engine.dispose()

    def full_page(_path, **kwargs):
        assert kwargs["params"]["start"] == "0"
        return [_activity(i + 200) for i in range(ACTIVITY_PAGE_SIZE)]

    client.responses["connectapi"] = full_page
    monkeypatch.setattr("healthcheck.garmin.sync.MAX_ACTIVITY_PAGES", 1)
    second = _run(tmp_path, client, max_provider_requests=10)
    activity = next(item for item in second.attempts if item.surface == "activities")
    assert second.status is GarminSyncStatus.PARTIAL
    assert activity.status is GarminSyncStatus.PARTIAL
    assert activity.coverage_status == "unknown"
    ...
    try:
        with factory() as session:
            ...
            assert (state.cursor, state.watermark, state.last_success_at) == previous
