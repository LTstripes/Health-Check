...
def test_difference_only_with_public_verified_flag_is_still_not_strong():
    result = compute_agreement_statistics(
        [
            AgreementObservation(
                wake_date=date(2099, 1, 1) + timedelta(days=index),
...
def test_projection_evidence_can_verify_difference_without_source_values():
    projections = [
        SimpleNamespace(
            pair=SimpleNamespace(
                wake_date=date(2099, 1, 1) + timedelta(days=index),
...
def test_auxiliary_metrics_never_become_canonical_proposals(metric_code):
    result = compute_agreement_statistics(_observations([0] * 42, metric_code=metric_code))
...
def test_non_default_epoch_requires_typed_real_break_basis():
    ...
    observation = AgreementObservation(
        ...
        epoch_basis=EpochBasis("device", "device-registry:garmin-vivoactive-5"),
        difference=0,
    )
    assert observation.epoch == "device-epoch-a"
    assert observation.epoch_basis is not None
    assert observation.epoch_basis.break_kind == "device"


def test_source_values_reject_inconsistent_supplied_difference_and_compute_missing_difference():
    ...
    assert observation.difference_verified is True
...
def test_issue_102_persistence_surface_coexists_with_issue_103_statistics_surface():
    assert PersistedSleepAgreementService is not None
    result = compute_agreement_statistics(_observations([0]))
    assert result.n == 1
